    ...
    public class MB_BatchPrefabBakerEditorFunctions
    {
        ...
        public static void BakePrefabs(MB3_BatchPrefabBaker pb, bool doReplaceTargetPrefab)
        {
            ...
            for (int prefabIdx = 0; prefabIdx < pb.prefabRows.Length; prefabIdx++)
            {
                ...
                {
                    if (MBVersion.GetMajorVersion() >= 2018 && MBVersion.GetMinorVersion() >= 3) _ProcessPrefabRowReplaceTargetPrefab(pb, pb.prefabRows[prefabIdx], tbr, unityTransforms, mb);
                    else _ProcessPrefabRowOnlyMeshesAndMaterials(pb, pb.prefabRows[prefabIdx], tbr, unityTransforms, mb);
                }
                else
                {
                    if (MBVersion.GetMajorVersion() >= 2018 && MBVersion.GetMinorVersion() >= 3) _ProcessPrefabRowOnlyMeshesAndMaterials(pb, pb.prefabRows[prefabIdx], tbr, unityTransforms, mb);
                    else _ProcessPrefabRowReplaceTargetPrefab_Legacy_2018_2_AndOlder(pb, pb.prefabRows[prefabIdx], tbr, unityTransforms, mb);
                }
            }
